...
class Selection {
    ...
    private _selectRange(range: Range) {
        ...
        if (!selection)
            return;
        // Guard: Chromium emits a browser-level console warning (not a
        // catchable exception) when addRange receives a range whose start/end
        // nodes are not yet attached to the document. Check both boundaries
        // before touching the selection API.
        const { startContainer, endContainer } = range;
        if (
            !this.doc.contains(startContainer)
            || !this.doc.contains(endContainer)
        ) {
            return;
        }
        ...
        try {
            ...
            // Should be unreachable now that we guard above, but keep as
            // fallback in case other edge cases arise.
        }
    }
    ...
    private _setFocus(focusNode: Node, focusOffset: number) {
        const selection = this.doc.getSelection();
        if (selection && this.doc.contains(focusNode)) {
