...
// Materialise all prism language components at build time via
// import.meta.glob. Vite cannot resolve pure variable-dynamic imports like
// `import(\`../../../node_modules/prismjs/components/prism-xx.js\`)` at
// runtime, so we pre-resolve every possible component to a loader function
// here. We intentionally do NOT use `eager` — languages must be loaded in
// dependency order (e.g. arduino depends on cpp, which depends on c, which
// depends on clike). The caller (via prismjs/dependencies) drives the order.
const prismModules = import.meta.glob(
    '/node_modules/prismjs/components/prism-*.js',
);

function findPrismModule(lang: string) {
    const key = `/node_modules/prismjs/components/prism-${lang}.js`;
    return prismModules[key];
}
    ...
    // If no argument is passed, load all components
        ...
        getLoader(components, langs, loaded).load(async (lang: string) => {
            ...
            if (!(lang in components.languages)) {
                ...
                // Resolve from the pre-materialised module map produced by
                // import.meta.glob above. Each entry is a dynamic-import
                // function — `() => Promise<module>`. We invoke it at the
                // right moment so dependencies are loaded in the order
                // prescribed by `getLoader`.
                const loader = findPrismModule(lang);
                if (loader) {
                    await loader();
                }
                defer.resolve({
